...
# 提示用户输入 node 节点 IP 和名称
read -p "请输入 node 节点 IP: " NODE_IP
read -p "请输入 node 节点名称（如 k8snode1）: " NODE_NAME
...
cfssl gencert \
  -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json \
  -hostname=${NODE_NAME},${NODE_IP} -profile=kubernetes ${NODE_NAME}-csr.json | cfssljson -bare ${NODE_NAME}
...
# 获取 node 节点的 IP 和用户名
read -p "请输入 node 节点 IP: " NODE_IP
read -p "请输入 node 节点用户名: " NODE_USER
